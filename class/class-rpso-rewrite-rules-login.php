<?php
    ...
    class RPSO_Rewrite_Rules_Login
    {
        ...
        public function rpso_login_rewrite_rules()
        {
            add_rewrite_tag('%rpso_auth%', '([^&]+)');
            add_rewrite_rule( '^rpso-verify-login/([^/]*)/?', 'index.php?rp_api=1&rpso_auth=$matches[1]', 'top' );
        }
        ...
        public function parse_request(&$wp)
        {
            if (array_key_exists('rpso_api', $wp->query_vars)) {
                include plugin_dir_path(__FILE__) . 'templates/rpso-verify-login.php';
                exit();
            }
        }
    }
